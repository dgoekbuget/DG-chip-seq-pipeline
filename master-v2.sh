...
qsub -t 1-"$nsamples" "$scripts"/fastq-dump.sh "$workdir" "$samples"

#Run mapping ($1=wordir, $2=GENOME, $3="--nodedup")
qsub -t 1-"$nsamples" -hold_jid "DG-fastqdmp" "$scripts"/mapping-v2.sh "$workdir" "$GENOME" "$samples"

#Convert to .bw
qsub -t 1-"$nsamples" -hold_jid "DG-mapping" "$scripts"/bam2bw-v2.sh "$workdir" "$samples"

#Cross-crosscorrelation
qsub -t 1-"$nsamples" -hold_jid "DG-mapping" "$scripts"/macs2_predictd.sh "$workdir" "$samples"
