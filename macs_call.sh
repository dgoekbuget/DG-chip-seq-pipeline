...
tr '\r' '\n' < $samples | sed -e '$a\' > "$samples"_temp
mv "$samples"_temp "$samples"
samples="$workdir"/"$samplesheet"

#Extract variables (nsamples = no. total samples, )
...
if [[ "$nctr" -ne 0 ]]; then
  for i in "${treatments[@]}"; do
    ...
    for (( j = 0; j < "$ntreat"; j++ )); do
      ...
      echo "MACS2 call: qsub -hold_jid "DG-bam2bw" $scripts/macs2.sh $optList2 ${optList1[@]} -w $workdir"
    done
  done
fi
