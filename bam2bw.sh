...
workdir=$1
files=( "$workdir"/bam/*.sorted.dedup.bam) # Expand the glob, store it in an array (Charles Duffy)
...
if [[ "$?" -eq 0 ]]; then
  rm "${infile%.sorted.dedup.bam}.dedup.unsorted.bedGraph"
	rm "${infile%.sorted.dedup.bam}.dedup.sorted.bedGraph"
